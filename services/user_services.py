...
# def update_default_address(request, authorize: AuthJWT, db:Session):
#     authorize.jwt_required()
#     db_user_update = db.query(user_models.User).filter(user_models.User.id == request.customer_id).first()

#     if not db_user_update:
#         common_msg = user_schemas.ResponseCommonMessage(status = status.HTTP_404_NOT_FOUND, message="User Doesn't Exist!")
#         return common_msg
...
#     db.merge(db_user_update, db_user_update.updated_at)
#     db.commit()
#     res_data= user_schemas.UpdateDefaultAddress(customer_id = request.customer_id,delivery_house_no_building_name = request.delivery_house_no_building_name,delivery_road_name_Area = request.delivery_road_name_Area,delivery_landmark = request.delivery_landmark,delivery_country = request.delivery_country,delivery_region = request.delivery_region,delivery_town_city = request.delivery_town_city,deliveryAddress_latitude = request.deliveryAddress_latitude,deliveryAddress_longitude = request.deliveryAddress_longitude)
#     common_msg = user_schemas.ResponseDefaultaddress(status = status.HTTP_200_OK, message="Default Address Updated Successfully !", data= res_data)
#     return common_msg
...
#         same_order_ref_no = db.query(user_models.Orders).filter(
#             user_models.Orders.ref_number == ref_no).first()
#         if same_order_ref_no:
#             ref_no = random.randint(999999, 99999999)
#         order = user_models.Orders(
#             ref_number=ref_no,
...
#         order_data = db.query(user_models.Orders).filter(
#             user_models.Orders.ref_number == order.ref_number).first()
...
#             if discount_unit == '%':
#                 discount_value = (product_price/100)*product_discount
...
#             discount_list.append(disc_with_qty)

#             if margin_unit == '%':
#                 margin_value = (product_price/100) * product_margin
#                 product_with_margin_price = product_price + margin_value
...
#             inventory = user_models.Inventory(
#                 order_id=order_id, quantity=product_qty, inventory_type_id=3, product_id=product_id, warehouse_id=1)
#             db.merge(inventory)
#             db.commit()
...
#         transaction_data = db.query(user_models.OrderTransactions).all()
#         if transaction_data:
#             inv_no = len(transaction_data)+1001
#         else:
#             inv_no = 1001
#         order_transact = user_models.OrderTransactions(user_id=request.customer_id,  order_id=order_id, order_type=1,
#                                                        payment_mode_id=request.payment_type, payment_status_id=request.payment_status, invoices_id=inv_no)
#         db.merge(order_transact)
#         db.commit()

#         order_delivery_logs = user_models.OrderDeliveryLogs(
#             order_id=order_id, order_status_id=1, order_status_details="Order is confirmed", user_id=1, customer_view=1)
#         db.merge(order_delivery_logs)
#         db.commit()

#         final_sub_total = sum(sub_total_list)
#         final_discount_total = sum(discount_list)
#         final_margin_total = sum(margin_list)
#         final_total = final_sub_total - final_discount_total
...
#         db.merge(order_data)
#         db.commit()
#         cart_items = db.query(user_models.CustomerCart).filter(
#             user_models.CustomerCart.customer_id == request.customer_id).all()
...
#         values = {
#             'order_number': ref_no,
#             'link': f"http://15.185.103.226/orders/{order_id}"
#         }
#         body = body.format(**values)
#         for to in send_emails_to:
#             common_services.send_otp(to, subject, body, request, db)

#         response = user_schemas.OrderResponse(
#             status=status.HTTP_200_OK, message="Order Placed Successfully")
#         return response
#     else:
#         common_msg = user_schemas.ResponseCommonMessage(
#             status=status.HTTP_200_OK, message="User is not approved to place the order")
#         return common_msg

# def processing_recurrence_order(db: Session):
#     all_recurrent_orders = db.execute(f"SELECT * FROM {constants.Database_name}.recurrence_grocery t1 inner join  {constants.Database_name}.grocery_products t2  on t2.grocery_id = t1.grocery_id ;")
#     for order in all_recurrent_orders:
#         customer_id = order.customer_id
#         grocery_id = order.grocery_id
#         recurrenttype = order.recurrenttype
#         recurrence_startdate = order.recurrence_startdate
#         recurrence_nextdate = order.recurrence_nextdate


#         user = db.query(user_models.User).filter(user_models.User.id == customer_id).first()
#         first_name = user.first_name
#         last_name = user.last_name
#         email = user.email
#         contact = user.contact
#         country = user.delivery_country
#         city = user.delivery_town_city
#         billing_name = first_name + last_name
#         billing_address = "have to look into it"
#         shipping_name = first_name + last_name
#         shipping_address = "have to look into it"
#         payment_type= 10
#         payment_status= 6
