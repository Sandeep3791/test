...
class ProductForecastLiberary:
    def __init__(self):
        ...
        self.FORECAST_PRODUCT_THRESHOLD=self.get_setting_value(FORECAST_PRODUCT_THRESHOLD)
    ...
    def process_forecast_product(self):
        try:
            ...
            for next_date_key,next_date_value in next_date_dict.items():
                get_product_quantity=self.get_grocery_products(next_date_value)
    ...
    def forecast_product(self,jobtype,get_product_quantity_dic):
        if get_product_quantity_dic:
            for get_product_quantity in get_product_quantity_dic:
                ...
                needed_stock_purchase=self.product_needed(stock,forecast_quantity,onhand_stock,outofstock_product)
                current_datetime = datetime.now()
                if stock >= 0:
                    check_stock=stock
                else:
                    check_stock=0
                forecast_product_dic={'forecast_jobtype_id':jobtype,'product_id':product_id,'stock':check_stock,'forecast_quantity':forecast_quantity,'needed_stock_purchase':needed_stock_purchase,'created_at':current_datetime}
                self.insert_forecast_product(forecast_product_dic)
        return 1

    def insert_forecast_product(self,forecast_product_dic):
        self.check_forecast_exist(forecast_product_dic) # check whether product exist for today date or not
    ...
    def check_forecast_exist(self,forecast_product_dic):
        ...
        is_forcast_product=ForecastProduct.objects.values('id').filter(forecast_jobtype_id=jobtype,product_id=product_id,created_at__gte=start_date_time,created_at__lte=last_date_time).first()
    ...
    def get_grocery_products(self,next_date_value):
        get_grocery_products=GroceryProducts.objects.annotate(total_product_qty=Sum('product_qty'),onhand=F('product__inventory_onhand'),outofstock=F('product__outofstock_threshold')).values('product','onhand','total_product_qty','outofstock').filter(recurrence_nextdate__gte=next_date_value['start_date'],recurrence_nextdate__lte=next_date_value['next_date'])
    ...
    def get_job_type(self):
        ...
        for job_type in job_type_dic:
            ...
            jobtype[no_of_days]={'start_date':date.today(),'next_date':next_date}
        return jobtype
