...
class Order(ApiBase):
    ...
    def create_order(self, order_details, account_code):
        ...
        response = ApiBase.send_request(
            self, method, path, create_order_list, headers, "json")
        return response
    ...
    def shipping_box(self, order_id):
        ...
        for orderdetail in orderdetails:
            shipping = {}
            total_amount = ((float(orderdetail.price) + float(orderdetail.item_margin))
                            * float(orderdetail.quantity))-float(orderdetail.discount)
