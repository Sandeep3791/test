...
from wayrem_admin.models.permissions import FunctionMaster, RolePermissions
...
class RolePermissionView(View):
    ...
    def post(self, request, *args, **kwargs):
        role_id = self.kwargs['id']
        role = get_object_or_404(Roles, pk=role_id)
        exist_permission = RolePermissions.objects.select_related(
            'function').filter(role_id=role.id).all()
        exist_permission = list(
            exist_permission.values_list('function_id', flat=True))
        ...
        if list_to_add:
            for function_id in list_to_add:
                bulk_list_to_add.append(RolePermissions(
        ...
        if list_to_remove:
            RolePermissions.objects.filter(Q(role_id=role_id) & Q(
                function__in=list_to_remove)).delete()
